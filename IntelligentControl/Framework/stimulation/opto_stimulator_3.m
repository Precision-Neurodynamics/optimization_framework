    ...
    methods
        ...
        function generate_stimulation_signal(obj)
            ...
            switch obj.stimulation_type, 
                case 'biphasic'
                    dneg = 1;
                    signal = opto_generate_biphasic(obj.sampling_frequency,  ...
                        obj.stimulation_frequency_pulse,                      ...
                        obj.stimulation_duration,                       ...
                        obj.stimulation_amplitude,                      ...
                        obj.stimulation_amplitude/dneg,                      ...
                        obj.stimulation_pulse_width_pulse,                    ...
                        obj.stimulation_pulse_width_pulse*dneg,                    ...
...
%                         size(obj.stimulation_channels,1));
                ...
                case 'train'
                    signal = opto_generate_train_boundary(obj.sampling_frequency,       ...
                ...
                case 'arbitrary'
                    signal = opto_generate_arbitrary(obj.sampling_frequency,                     ...
                        ...
                        size(obj.stimulation_channels,1));
                case 'sequence'
                    signal = opto_generate_sequence(obj.sampling_frequency,                     ...
                        obj.stimulation_duration,                           ...
                        obj.stimulation_amplitude,                        ...
                        obj.stimulation_frequency_pulse,                    ...
                        obj.stimulation_pulse_width_pulse,                        ...
...
%                             posixtime(datetime)
        ...
        function log_stimulation(obj)
            ...
            if strcmp(obj.stimulation_type,'sequence')
                for i1=1:1:length(obj.stimulation_frequency_pulse)-1
                    log_string = [log_string sprintf('%f-', obj.stimulation_frequency_pulse(i1))];
                end
                log_string = [log_string sprintf('%f,', obj.stimulation_frequency_pulse(i1+1))];
            else
                log_string = [log_string sprintf('%f,', obj.stimulation_frequency_pulse)];
            end
            log_string = [log_string sprintf('%f,', obj.stimulation_duration)];
            if strcmp(obj.stimulation_type,'sequence')
                for i1=1:1:length(obj.stimulation_amplitude)-1
                    log_string = [log_string sprintf('%f-', obj.stimulation_amplitude(i1)/2)];
                end
                log_string = [log_string sprintf('%f,', obj.stimulation_amplitude(i1+1)/2)];
                for i1=1:1:length(obj.stimulation_amplitude)-1
                    log_string = [log_string sprintf('%f-', obj.stimulation_amplitude(i1)/2)];
                end
                log_string = [log_string sprintf('%f,', obj.stimulation_amplitude(i1+1)/2)];
            ...
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            if strcmp(obj.stimulation_type,'sequence')
                fprintf('Frequency_pulse: %.1f-',obj.stimulation_frequency_pulse(1))
                for i1=2:1:length(obj.stimulation_frequency_pulse)-1
                    fprintf('%.1f-',obj.stimulation_frequency_pulse(i1))
                end
                fprintf('%.1f',obj.stimulation_frequency_pulse(end));
                fprintf(', Amplitude: %.1f-',obj.stimulation_amplitude(1))
                for i1=2:1:length(obj.stimulation_amplitude)-1
                    fprintf('%.1f-',obj.stimulation_amplitude(i1))
                end
                fprintf('%.1f',obj.stimulation_amplitude(end));
                fprintf(', Frequency_train: %.1f Duration: %.2f, ', ...
                    obj.stimulation_frequency_train, obj.stimulation_duration);
            else
