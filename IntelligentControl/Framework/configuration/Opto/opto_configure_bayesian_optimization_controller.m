...
experiment_name                             = 'Theta-train-delta';%'PiSM';%'Train_opt_IISremove';%'IIS_test';'4-10Hz_fitmax_2param_Freq_PW_OF_area_delta_trial1';%'6-10Hz_max_2param_OF_delta_trial1';%'test_contoller_2';%
...
recording_channels  = [1:12 14:16];%[1:16];%[2 4 6 8 9 11 13 15];
...
metric              = opto_model_state(sampling_frequency, recording_channels, metric_def, metric_type, []);
metric_objects      = {metric};
deltaorraw          = 'raw';%'complex';%'delta';
maxormin            = 'maximize';
...
if strcmp(stimulation_manager.stimulation_type,'standard')
    lower_bound         = [   2 1*10^-3];%[35 5];%
    upper_bound         = [100 8*10^-3];%[100 11];%
    stimulation_manager.stimulation_pulse_width_pulse   = 0;%4*10^-3;%
    stimulation_manager.stimulation_pulse_width_train   = 1;%75*10^-3;%
    optimparam = {'pulse_frequency','pulse_width_pulse'};%{'pulse_frequency','train_frequency'};%
...
stimulation_manager.stimulation_duration    = 8;
optimization_object.objective_window_s      = stimulation_manager.stimulation_duration;
optimization_object.evaluate_delay_s        = 8;
optimization_object.stimulator              = stimulation_manager;

optimization_object.n_burn_in               =10; % = 10 originally

% gp_folder = 'C:\Users\TDT\Documents\IntelligentControl\results\EPI043\bayesian_optimization\PiSM-train-complex-EPI043_20190801T160537\';
% gp_model = load([gp_folder,'\gp_model.mat']);
% user_define_length = 19;
