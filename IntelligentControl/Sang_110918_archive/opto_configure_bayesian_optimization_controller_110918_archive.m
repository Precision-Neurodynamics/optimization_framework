function [optimization_object, stimulation_manager, metric_objects] = opto_configure_bayesian_optimization_controller_110918_archive( TD, DEBUG)

temp = datestr(now,2);
temp(strfind(temp,'/')) =[]; % Today date
animal_id                                   = 'EPI033';
result_dir                                  = ['results/' animal_id '/bayesian_optimization'];
experiment_name                             = 'Train_50I-test';%'Train_opt_IISremove';%'IIS_test';'4-10Hz_fitmax_2param_Freq_PW_OF_area_delta_trial1';%'6-10Hz_max_2param_OF_delta_trial1';%'test_contoller_2';%
...
stimulation_manager.stimulation_pulse_width_train   = 75*10^-3;

% stimulation_manager.stimulation_pulse_width = 1.3; % 820 microsecond pulse-width
            
stimulation_manager.stimulation_type        = 'train';%'train';%
...
gp_model.lower_bound            = [35 5];%[   2 1*10^-3];%
gp_model.upper_bound            = [100 11];%[100 8*10^-3];% %EPI018 : 4.4, %EPI016 : 4.6, %EPI015 : 5 (based on commutator) EPI019 : 4.2
...
optimization_object.stimulation_parameter   = {'pulse_frequency','train_frequency'};%{'pulse_frequency','pulse_width_pulse'};%
